...
basemap<-ggplot(crime_map,aes(x=long,y=lat,group=group,fill=Murder)) + coord_map("polyconic") 

# Color map sequential increasing with colorspace
basemap + geom_polygon() + scale_fill_continuous_sequential(palette="Peach")

# Color map diverging from the midpoint (mean murder rate) with colorspace 
basemap + geom_polygon() + scale_fill_continuous_diverging(palette="Blue-Red",mid=mean(crime_map$Murder))

basemap + geom_polygon(fill=hsv((max(crime_map$Murder)-crime_map$Murder)/max(crime_map$Murder)/7,crime_map$Assault/max(crime_map$Assault),1))
